...
import com.example.jormun_map.model.data_classes.Batiment;
...
public class Activite_Village extends AppCompatActivity {
    ...
    public void Generation_village(final int rang)
    {
        ...
        ArrayList<Batiment> ListBat = vill.getListBat();
        Batiment batactu;
        VilNom.setText(vill.getNom());
        VilLvl.setText("Lvl"+ vill.getLvl());
        if(ListBat.size()>=(rang+1))
        {
            batactu = ListBat.get(0);
            btn_bat0.setBackgroundResource(batactu.getImage());
            ...
            if(ListBat.size()>=rang+2)
            {
                batactu = ListBat.get(1);
                btn_bat1.setBackgroundResource(batactu.getImage());
                ...
                if(ListBat.size()>=rang+3)
                {
                    batactu = ListBat.get(2);
                    btn_bat2.setBackgroundResource(batactu.getImage());
                    ...
                    if (ListBat.size()>=rang+4)
                    {
                        batactu = ListBat.get(3);
                        btn_bat3.setBackgroundResource(batactu.getImage());
                        ...
                        if(ListBat.size()>=rang+5)
                        {
                            batactu = ListBat.get(4);
                            btn_bat4.setBackgroundResource(batactu.getImage());
    ...
    public void AffichBatprec(final int Batplace, final Button btnCible, final Batiment bat)
    {
        btnCible.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Nom_bat.setText(bat.getNom() + " " + bat.getLvl());
                Nom_bat.setVisibility(View.VISIBLE);
                /////sprite = sprite bat
                SpriteBat.setVisibility(View.VISIBLE);
                SpriteBat.setImageResource(bat.getImage());
    ...
    {
        String image;
        if(batactu.getType() ==1)
        {
            if(batactu.getLvl() <4)
            {
                image="taverne";
            }
            else if (batactu.getLvl() >=7)
            {
    ...
    public void SetColorBat(Button btn_bat, Batiment bat_actu)    ////affiche si possible de LvlUp avec ressource actu
    {
        ...
        int LvlBat = bat_actu.getLvl();
        int typebat= bat_actu.getType();
        ...
        if(up_ok==true)
        {
            btn_bat.setBackgroundResource(bat_actu.getImage());
        }
    }
    public void SetUpTrue(final Button btn_bat, final Batiment bat_actu, TextView ressourceManq, final int Batplace)    ////affiche si possible de LvlUp avec ressource actu
    {
        ...
        else
        {
            ...
            btn_up_bat.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    bat_actu.setLvl(bat_actu.getLvl() + 1);
    ...
    {
        System.out.println("Verif ressources");
        boolean up_ok = true;
        boolean Find=false;
        ...
        for(int Compteur_Bat=0; Compteur_Bat< Bat_requis.size();Compteur_Bat++)
        {
            ...
            System.out.println("Batiment besoin de " + ressourceActu.getQuantite() + " de " + ressourceActu.getNom());
            for (int Compteur_inv = 0; Compteur_inv< actuUtilisateur.getInventaire_ressources().size(); Compteur_inv++)
            {
                ressourceInInv=  actuUtilisateur.getInventaire_ressources().get(Compteur_inv);
                System.out.println("Actu ressource = " + ressourceInInv.getNom()+ " quant = " + ressourceInInv.getQuantite());
                if(ressourceActu.getNom() == ressourceInInv.getNom())
                {
                    System.out.println("Joueur a  " + ressourceInInv.getQuantite());
                    if(ressourceActu.getQuantite() > ressourceInInv.getQuantite())
                    {
                        up_ok=false;
                        Manq = Manq + " "+ (ressourceActu.getQuantite() - ressourceInInv.getQuantite()) + " " + ressourceActu.getNom();
                    }
                    Find=true;
                }
            }
            if(Find==false)
            {
                Manq = Manq + " " + ressourceActu.getQuantite() + " " + ressourceActu.getNom();
                up_ok=false;
            }
        }
        return up_ok;
    ...
    public void AffichProd(Batiment batActu)
    {
        ...
        ArrayList<Ressources> prodSem = batActu.getProdSem();
        Ressources Ress;
        ...
        for (int Compteur=0; Compteur<prodSem.size();Compteur++)
        {
            ...
            Prod.setText(""+ Ress.getQuantite() +" "+ Ress.getNom() +" par semaine");
            Prod.setTextSize(15);
    ...
    public void CreateBatYEs(String Bat_nom,int type,int Lvl) ///+, Sprite
    {
        ...
        Verif_end_Ress(newBat, actuUtilisateur.getInventaire_ressources());
    ...
    public void Gen_prod_end(Village village)
    {
        ...
        for (int Compteur = 0; Compteur< village.getListBat().size(); Compteur++)
        {
            for (int Compteur2 = 0; Compteur2< village.getListBat().get(Compteur).getProdSem().size(); Compteur2++)
            {
                TextView Prod = new TextView(this);
                Prod.setText(""+ village.getListBat().get(Compteur).getProdSem().get(Compteur2).getQuantite() +" "+ village.getListBat().get(Compteur).getProdSem().get(Compteur2).getNom() +" par semaine");
                Prod.setTextColor(Color.rgb(219,223,223));
                Container.addView(Prod);
            }
        }
    ...
    public void EnleverRess(ArrayList<Ressources> RessDemandee)
    {
        ...
        for(int Compteur_Bat=0; Compteur_Bat< RessDemandee.size();Compteur_Bat++)
        {
            ...
            for (int Compteur_inv = 0; Compteur_inv< actuUtilisateur.getInventaire_ressources().size(); Compteur_inv++)
            {
                if(ressourceActu.getNom() == actuUtilisateur.getInventaire_ressources().get(Compteur_inv).getNom())
                {
                    actuUtilisateur.getInventaire_ressources().get(Compteur_inv).setQuantite(actuUtilisateur.getInventaire_ressources().get(Compteur_inv).getQuantite() - ressourceActu.getQuantite());
                }
            }
        }
    }
    ...
    public void Simulation()
    {
        ...
        Ressources boisdepart= new Ressources(124,"Bois"," ca pousse sur les arbres");
