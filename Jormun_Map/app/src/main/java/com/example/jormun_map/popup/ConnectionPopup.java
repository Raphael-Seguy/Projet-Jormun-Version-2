        ...
        getBtnConfirm().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View vwCurrent) {
                ...
                if (VerifyError((a_iInt = CheckInput()))) {
                    sAnswer = getExtdbmgrApiServer().ConnectUser(getEdttxtUsername().getText().toString(), getEdttxtPassword().getText().toString(), getRandomId());
                    if (!sAnswer.contains(ExternalDbManager.sFAILED)) {
                        shrdprefeditScribble = getShrdprefSave().edit();
                        shrdprefeditScribble.putString(MapsActivity.sUSERNAMEKEY, getEdttxtUsername().getText().toString());
