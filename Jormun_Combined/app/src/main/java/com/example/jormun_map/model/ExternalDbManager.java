...
public class ExternalDbManager {
    ...
    public void SendComplexeFile(TablesName tblCurrent, int iId,int iIdUser){

    }
    public void FetchComplexeFile(TablesName tblTable,int iId,int iIdUser){
    ...
    public void FetchSimpleFile(TablesName tblTable, int iId) {
        ...
        sReceivedAnswer = (String) RequestManagerWrapper(HttpRequestType.GET_REQUEST.toString(), DatabaseActions.ACTION_GRABFILE, hshmps_sSet)[0];
        if (sReceivedAnswer != null) {
            try{
                sReceivedAnswer = sReceivedAnswer.trim();
                if(sReceivedAnswer.charAt(0)=='{'){
